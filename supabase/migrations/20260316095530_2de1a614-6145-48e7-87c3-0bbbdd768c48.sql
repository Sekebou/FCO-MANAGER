...
-- Secure settle_match_bets: only admin_plus can call it
CREATE OR REPLACE FUNCTION public.settle_match_bets(p_home_team text, p_away_team text, p_match_date text, p_home_score integer, p_away_score integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_settled integer := 0;
BEGIN
  -- Only admin_plus can settle bets
