CREATE OR REPLACE FUNCTION public.place_bet(
  ...
  p_bet_type text DEFAULT 'match'::text,
  p_scorer_player_id uuid DEFAULT NULL::uuid,
  p_scorer_player_name text DEFAULT NULL::text,
  p_predicted_score_home integer DEFAULT NULL::integer,
  p_predicted_score_away integer DEFAULT NULL::integer
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- ============================================================
  -- ANTI-CHEAT: lock bets at kickoff + 5min
  -- Try to find the event in DB to get the exact kickoff time
  -- ============================================================
  ...
  IF v_event_record.date IS NOT NULL THEN
    -- Build timestamp from event date + time
    BEGIN
      ...
      ELSE
        -- No time → default lockout at 05:00 the match day (safe default)
    ...
    END;
  ELSE
    -- Fallback if no event row: lock at 05:00 of match day
    BEGIN
      v_kickoff_ts := (v_normalized_date || ' 05:00:00')::timestamptz;
    EXCEPTION WHEN OTHERS THEN
      v_kickoff_ts := NULL;
