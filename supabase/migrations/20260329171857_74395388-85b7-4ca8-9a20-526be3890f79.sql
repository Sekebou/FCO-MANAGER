...
-- Add team column to bets table
ALTER TABLE public.bets ADD COLUMN IF NOT EXISTS team text DEFAULT NULL;

-- Update place_bet RPC to include team parameter and use it for duplicate detection
CREATE OR REPLACE FUNCTION public.place_bet(p_user_id uuid, p_user_name text, p_match_date text, p_home_team text, p_away_team text, p_prediction text, p_odds numeric, p_amount integer, p_team text DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Check for existing bet on same match WITH team distinction
  ...
  WHERE user_id = p_user_id
    ...
    AND (
      -- Both have team: must match
      (team IS NOT NULL AND p_team IS NOT NULL AND team = p_team)
      -- Both null: legacy behavior
      OR (team IS NULL AND p_team IS NULL)
      -- One null, one not: allow (different matches)
    );
