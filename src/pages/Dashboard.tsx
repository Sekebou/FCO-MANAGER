...
export interface Event {
  ...
  convocationsPublishedByName?: string;
  convocationsPublishedAt?: string;
  createdBy?: string;
...
const mapEvent = (r: any): Event => ({ id: r.id, title: r.title, date: r.date, type: r.type, team: r.team, reason: r.reason, recurrence: r.recurrence, presences: r.presences as any || {}, absenceReasons: r.absence_reasons as any || {}, convocations: r.convocations as any || {}, convocationsPublished: r.convocations_published ?? false, convocationsPublishedByName: r.convocations_published_by_name || undefined, convocationsPublishedAt: r.convocations_published_at || undefined, createdBy: r.created_by, createdByName: r.created_by_name, createdAt: r.created_at, time: r.time, location: r.location, duration: r.duration ?? undefined, homeLogo: r.home_logo || undefined, awayLogo: r.away_logo || undefined });
...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-secondary/30 flex flex-col pb-24">
      {/* Header */}
      {/* Header */}
      <header className={`app-header sticky z-50 pt-[env(safe-area-inset-top)] transition-transform duration-300 ease-in-out lg:translate-y-0 lg:top-0 ${headerVisible ? 'top-0 translate-y-0' : 'top-0 -translate-y-full'}`}>
