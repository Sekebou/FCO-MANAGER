...
const ChatTab: React.FC<Props> = ({ currentUser }) => {
    ...
    const channel = supabase.channel('chat-tab')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'chat_messages' }, () => {
        supabase.from('chat_messages').select('*').order('created_at', { ascending: true }).limit(200)
          .then(({ data }) => data && setMessages(data.map(mapMsg)));
      })
      .subscribe();
  ...
  const handleDeleteMessage = async (msgId: string) => {
    try { await supabase.from('chat_messages').delete().eq('id', msgId); }
  ...
  const handleResetChat = async () => {
    ...
    } catch (err) { console.error('Error resetting chat:', err); toast.error('Erreur lors de la suppression'); }
      ...
      {showResetConfirm && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm rounded-2xl">
          <div className="bg-card border border-border rounded-2xl p-6 mx-4 max-w-sm w-full shadow-xl">
            ...
            <p className="text-sm text-muted-foreground mb-5">Tous les messages ({messages.length}) seront définitivement supprimés. Cette action est irréversible.</p>
            <div className="flex gap-2">
              <button onClick={() => setShowResetConfirm(false)} disabled={resetting} className="flex-1 px-4 py-2.5 bg-secondary text-foreground rounded-xl font-medium hover:bg-secondary/80 transition-all text-sm">Annuler</button>
              <button onClick={handleResetChat} disabled={resetting} className="flex-1 px-4 py-2.5 bg-destructive text-destructive-foreground rounded-xl font-medium hover:brightness-110 transition-all text-sm disabled:opacity-50">{resetting ? 'Suppression...' : 'Tout supprimer'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div ref={messagesContainerRef} className="flex-1 overflow-y-auto px-4 py-3 space-y-0.5 relative" style={{ scrollbarWidth: 'thin' }}>
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-muted-foreground gap-4 py-12">
            <div className="w-20 h-20 rounded-3xl bg-secondary/80 flex items-center justify-center"><MessageCircle size={36} className="opacity-40" /></div>
            <div className="text-center"><p className="text-sm font-medium">Aucun message pour l'instant</p><p className="text-xs mt-1 opacity-70">Soyez le premier à lancer la conversation ! 💬</p></div>
          </div>
        )}
            ...
            <React.Fragment key={msg.id}>
              {showDateSep && (
                <div className="flex items-center gap-3 py-3">
                  ...
                  <span className="text-[11px] font-medium text-muted-foreground bg-card px-3 py-1 rounded-full border border-border shadow-sm">{formatDateSeparator(msg.createdAt)}</span>
                  <div className="flex-1 h-px bg-border" />
                </div>
              )}
              <div className={`group flex items-end gap-2.5 ${own ? 'flex-row-reverse' : ''} ${consecutive ? 'mt-0.5' : 'mt-4'}`}>
                {!own && !consecutive ? (
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center text-[11px] font-bold text-white shrink-0 overflow-hidden shadow-sm ${ROLE_COLORS[msg.userRole] || 'bg-muted-foreground'}`}>
                    {msg.userPhoto ? <img src={msg.userPhoto} alt="" className="w-full h-full object-cover" /> : getInitials(msg.userName)}
                  </div>
                ) : !own ? <div className="w-9 shrink-0" /> : null}
                <div className={`max-w-[78%] flex flex-col ${own ? 'items-end' : 'items-start'}`}>
                  {!consecutive && !own && (
                    <div className="flex items-center gap-1.5 mb-1 ml-1">
                      <span className={`text-xs font-bold ${ROLE_TEXT_COLORS[msg.userRole] || 'text-muted-foreground'}`}>{msg.userName}</span>
                      <span className={`text-[9px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded-md ${ROLE_COLORS[msg.userRole] || 'bg-muted-foreground'} text-white opacity-80`}>{ROLE_LABELS[msg.userRole] || msg.userRole}</span>
                    </div>
                  )}
                  <div className="flex items-center gap-1.5">
                    {own && canDelete && <button onClick={() => handleDeleteMessage(msg.id)} className="opacity-0 group-hover:opacity-100 p-1 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all" title="Supprimer"><Trash2 size={13} /></button>}
                    <div className={`px-4 py-2.5 text-sm leading-relaxed break-words shadow-sm ${own ? 'bg-gradient-to-br from-accent to-accent/90 text-accent-foreground rounded-2xl rounded-br-lg' : 'bg-secondary text-foreground rounded-2xl rounded-bl-lg border border-border/50'}`}>{msg.text}</div>
                    {!own && canDelete && <button onClick={() => handleDeleteMessage(msg.id)} className="opacity-0 group-hover:opacity-100 p-1 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all" title="Supprimer"><Trash2 size={13} /></button>}
                  </div>
                  {!consecutive && <span className={`text-[10px] text-muted-foreground/70 mt-1 ${own ? 'mr-1' : 'ml-1'}`}>{formatTime(msg.createdAt)}</span>}
                </div>
              </div>
            </React.Fragment>
          );
        ...
        <div className="border-t border-border px-4 py-3 bg-card/80 backdrop-blur-sm">
          <div className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold text-white shrink-0 overflow-hidden ${ROLE_COLORS[currentUser.role] || 'bg-muted-foreground'}`}>
              {currentUser.photoURL ? <img src={currentUser.photoURL} alt="" className="w-full h-full object-cover" /> : getInitials(currentUser.name)}
            </div>
            <input ref={inputRef} type="text" value={newMessage} onChange={e => setNewMessage(e.target.value)} onKeyDown={handleKeyDown} placeholder="Écrivez un message..." className="flex-1 px-4 py-2.5 bg-secondary border border-border rounded-2xl text-foreground placeholder:text-muted-foreground outline-none focus:ring-2 focus:ring-accent/40 focus:border-accent/40 text-sm transition-all" maxLength={1000} />
            <button onClick={handleSend} disabled={!newMessage.trim() || sending} className="p-2.5 bg-gradient-to-br from-accent to-accent/80 text-accent-foreground rounded-2xl hover:shadow-md hover:scale-105 transition-all disabled:opacity-30 disabled:cursor-not-allowed disabled:hover:scale-100 disabled:hover:shadow-none"><Send size={18} /></button>
          </div>
        </div>
      ) : (
        <div className="border-t border-border px-4 py-4 text-center text-sm text-muted-foreground bg-secondary/50">🔒 Connectez-vous pour participer à la discussion</div>
      )}
    </div>
  );
};
