...
const ParisTab: React.FC<Props> = ({ currentUser, championships }) => {
  ...
  return (
    <div className="space-y-4">
      {/* ── Hero Header — 2026 native football ── */}
      <div className="relative overflow-hidden rounded-[28px] shadow-xl shadow-primary/25 ring-1 ring-white/10">
        {/* Pitch background */}
        <div
          className="absolute inset-0 bg-cover bg-center"
          style={{ backgroundImage: `url(${parisHeroBg})` }}
        />
        {/* Deep navy tint for legibility */}
        <div className="absolute inset-0 bg-gradient-to-b from-[#0a1854]/80 via-[#0a1854]/72 to-[#06103d]/92" />
        {/* Center pitch line glow (subtle) */}
        <div className="absolute inset-y-0 left-1/2 -translate-x-px w-px bg-gradient-to-b from-transparent via-white/15 to-transparent pointer-events-none" />
        {/* Top scanline accent */}
        ...
        <div className="relative p-4">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-xl bg-white/8 backdrop-blur-xl border border-white/15 flex items-center justify-center shadow-inner shadow-white/5">
                <Trophy className="text-white" size={15} strokeWidth={2.5} />
              </div>
              <div className="leading-tight">
                <p className="text-[9px] font-bold text-white/55 uppercase tracking-[0.22em]">Pronostics</p>
                <h2 className="text-[15px] font-black text-white tracking-tight">Mes Paris</h2>
              </div>
            </div>
            <button
              onClick={handleForceRefresh}
              disabled={refreshing || currentData.loading}
              className="w-9 h-9 rounded-xl bg-white/8 backdrop-blur-xl border border-white/15 flex items-center justify-center active:scale-95 transition-all disabled:opacity-40"
              title="Actualiser les matchs"
            >
              <RefreshCw size={13} className={cn("text-white", (refreshing || currentData.loading) && "animate-spin")} />
            </button>
          </div>

          {/* Scoreboard row : Solde / En cours */}
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div>
              <p className="text-[9px] font-bold text-white/50 uppercase tracking-[0.2em] mb-1.5">Solde</p>
              <div className="flex items-baseline gap-1.5">
                <Coins size={18} className="text-amber-300 self-center drop-shadow-[0_0_8px_rgba(252,211,77,0.5)]" />
                <span className="text-[32px] font-black text-white leading-none tabular-nums tracking-tight">{balance}</span>
                <span className="text-[10px] font-bold text-white/55 ml-0.5">PTS</span>
              </div>
            </div>
            {myPendingBets.length > 0 && (
              <div className="text-right border-l border-white/10 pl-3">
                <p className="text-[9px] font-bold text-white/50 uppercase tracking-[0.2em] mb-1.5">En cours</p>
                <div className="flex items-baseline justify-end gap-1.5">
                  <span className="text-[32px] font-black text-white leading-none tabular-nums tracking-tight">{myPendingBets.length}</span>
                  <span className="text-[10px] font-bold text-white/55">{myPendingBets.length > 1 ? 'PARIS' : 'PARI'}</span>
                </div>
              </div>
            )}
          </div>
          ...
          {/* Stats row — football scoreboard style */}
          <div className="grid grid-cols-3 divide-x divide-white/10">
            <div className="text-center px-1">
              <div className="text-[22px] font-black text-white leading-none tabular-nums tracking-tight">{myBets.length}</div>
              <div className="text-[8.5px] font-bold text-white/50 uppercase tracking-[0.18em] mt-1.5">Total</div>
            </div>
            <div className="text-center px-1">
              <div className="text-[22px] font-black text-white leading-none tabular-nums tracking-tight">
                {myWonBets.length}
                <span className="inline-block w-1 h-1 rounded-full bg-emerald-400 ml-1 align-middle shadow-[0_0_6px_rgba(52,211,153,0.8)]" />
              </div>
              <div className="text-[8.5px] font-bold text-white/50 uppercase tracking-[0.18em] mt-1.5">Gagnés</div>
            </div>
            <div className="text-center px-1">
              <div className="text-[22px] font-black text-white leading-none tabular-nums tracking-tight">
                {myLostBets.length}
                <span className="inline-block w-1 h-1 rounded-full bg-rose-400/80 ml-1 align-middle" />
              </div>
              <div className="text-[8.5px] font-bold text-white/50 uppercase tracking-[0.18em] mt-1.5">Perdus</div>
            </div>
          </div>
        </div>
      </div>
        ...
        {activeFilter === 'upcoming' && (
          <motion.div key="upcoming" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
            {currentData.loading ? (
              ...
              return (
                <>
                  {/* Match card — premium minimal & compact */}
                  <motion.div
                    ...
                    <div className="relative px-4 py-4">
                      ...
                      <div className="mb-3 flex items-center justify-between gap-2">
                        <div className="flex min-w-0 items-center gap-2">
                          ...
                          <div className="min-w-0">
                            ...
                            <h3 className="truncate text-[13px] font-black text-foreground leading-tight mt-0.5">
                              Équipe {selectedTeam}
                            </h3>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-1.5">
                          {live && (
                            <span className="flex items-center gap-1 rounded-full bg-gradient-to-r from-red-600 to-red-500 px-2 py-0.5 text-[9px] font-black uppercase tracking-wider text-white animate-pulse">
                              <span className="h-1.5 w-1.5 rounded-full bg-white animate-ping" />
                              LIVE
                            </span>
                          )}
                          {waiting && (
                            <span className="flex items-center gap-1 rounded-full bg-amber-500/10 px-2 py-0.5 text-[9px] font-semibold text-amber-600">
                              <Clock size={9} />
                              Attente
                            </span>
                          )}
                      ...
                      <div className="mb-3 flex items-center justify-center gap-3">
                        <div className="flex flex-1 flex-col items-center gap-1.5">
                          {homeLogo ? (
                            <img src={homeLogo} alt="" className="h-12 w-12 rounded-full object-cover ring-1 ring-border/60" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                          ) : <div className="h-12 w-12 rounded-full bg-muted" />}
                          <span className={`text-center text-[11px] font-bold leading-tight line-clamp-2 ${nextMatch.home?.club?.cl_no === OISEMONT_CL_NO ? 'text-primary' : 'text-foreground'}`}>
                            {homeName}
                          </span>
                          {homeRank && <span className="text-[9px] font-medium text-muted-foreground">{homeRank}e</span>}
                        </div>

                        <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary">
                          <span className="text-[13px] font-black">VS</span>
                        </div>

                        <div className="flex flex-1 flex-col items-center gap-1.5">
                          {awayLogo ? (
                            <img src={awayLogo} alt="" className="h-12 w-12 rounded-full object-cover ring-1 ring-border/60" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                          ) : <div className="h-12 w-12 rounded-full bg-muted" />}
                          <span className={`text-center text-[11px] font-bold leading-tight line-clamp-2 ${nextMatch.away?.club?.cl_no === OISEMONT_CL_NO ? 'text-primary' : 'text-foreground'}`}>
                            {awayName}
                          </span>
                          {awayRank && <span className="text-[9px] font-medium text-muted-foreground">{awayRank}e</span>}
                        </div>
                      </div>
                        ...
                        <div className="mb-3 flex items-center justify-center gap-1">
                          {[
                            ...
                            <React.Fragment key={c.label}>
                              {i > 0 && <span className="text-[10px] font-black text-primary/30">:</span>}
                              <div className="flex items-baseline gap-0.5 rounded-lg bg-secondary px-2 py-1">
                                <span className="text-[13px] font-black leading-none text-foreground tabular-nums">{String(c.val).padStart(2, '0')}</span>
                                <span className="text-[8px] font-bold uppercase text-muted-foreground">{c.label}</span>
                              </div>
                            </React.Fragment>
                          ))}
                        </div>
                      ...
                      {waiting && (
                        <div className="mb-3 flex items-center justify-center gap-1.5 py-1">
                          ...
                          <span className="text-[11px] font-semibold text-amber-600">En attente du résultat</span>
                        </div>
                      )}

                      {/* Date */}
                      <p className="mb-3 text-center text-[11px] font-medium text-muted-foreground">
                        {nextMatch.date ? new Date(nextMatch.date).toLocaleDateString('fr-FR', { weekday: 'short', day: 'numeric', month: 'short' }) : ''}
                      ...
                      {currentUser && !live && !waiting && (
                        <motion.button
                          ...
                          className="flex w-full items-center justify-center gap-1.5 rounded-xl bg-primary py-2.5 text-[12px] font-bold text-primary-foreground shadow-sm transition-all hover:brightness-110"
                        >
                          <Zap size={13} />
                      ...
                      {locationLabel && (
                        <div className="mt-2.5 flex items-center justify-center gap-1">
                          ...
                          {locationLink ? (
                            <a href={locationLink} target="_blank" rel="noopener noreferrer" className="flex max-w-[250px] items-center gap-1 truncate text-[10px] font-medium text-primary underline underline-offset-2">
                              {locationLabel} <ExternalLink size={9} />
                            </a>
                          ) : (
                            <span className="truncate text-[10px] text-muted-foreground">{locationLabel}</span>
                          )}
                        </div>
                      )}
                    </div>
