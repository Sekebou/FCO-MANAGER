          ...
          <div className="p-4 pb-3 border-b border-border/50 sticky top-0 bg-background/95 backdrop-blur-md z-10 flex items-center gap-3">
            ...
            {!selectedPlayerId && (
              ...
              >
                ...
                <span className="text-[11px] font-semibold text-accent">Exporter stats globales</span>
              </button>
            )}
            {selectedPlayerId && (
              <button
              ...
              >
                <Download size={14} className="text-accent" />
                <span className="text-[11px] font-semibold text-accent">Récap joueur</span>
              </button>
            )}
          </div>
                    ...
                    return (
                      <>
                        ...
                        {playerCards.length === 0 ? (
                          ...
                          <>
                            {/* Show only the most recent card */}
                            {(() => {
                              const mostRecent = [...playerCards].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
                              return (
                                <div className={`flex items-center gap-3 p-3 rounded-xl border ${mostRecent.type === 'yellow' ? 'bg-warning/5 border-warning/20' : 'bg-destructive/5 border-destructive/20'}`}>
                                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${mostRecent.type === 'yellow' ? 'bg-warning/20' : 'bg-destructive/20'}`}>
                                    <AlertTriangle size={14} className={mostRecent.type === 'yellow' ? 'text-warning' : 'text-destructive'} />
                                  </div>
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2">
                                      <span className={`text-xs font-bold ${mostRecent.type === 'yellow' ? 'text-warning' : 'text-destructive'}`}>
                                        {mostRecent.type === 'yellow' ? 'JAUNE' : 'ROUGE'}
                                      </span>
                                      <span className="text-[10px] text-muted-foreground flex items-center gap-1">
                                        <Calendar size={10} /> {new Date(mostRecent.date).toLocaleDateString('fr-FR')}
                                      </span>
                                    </div>
                            ...
                            >
                              <Eye size={13} />
                              Historique complet ({playerCards.length} carton{playerCards.length > 1 ? 's' : ''})
                            </button>
                          </>
                        )}
                      </>
