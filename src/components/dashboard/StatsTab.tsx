              ...
              {[1, 0, 2].map((podiumIdx) => {
                ...
                return (
                  <div key={item.player.id} className={`flex flex-col items-center p-4 rounded-2xl border ${podiumBgs[podiumIdx]} ${isFirst ? 'scale-105 shadow-md' : ''} transition-all`}>
                    ...
                    <div className="text-[10px] text-muted-foreground mt-0.5">Venu à {item.attendance!.present} entraînement{item.attendance!.present > 1 ? 's' : ''} sur {item.attendance!.total}</div>
                  </div>
                );
              })}
            </div>
